...
const ForumWidgetComponent: React.FC<ForumWidgetProps> = (propsIn) => {
    ...
    const fetchCallback = useCallback((data: IForumData) => {
        ...
        setLoading(false);
        setFade(false);
        setNextInterval(data.nextInterval);
    }, [activeKey, forTabs, forumPrevStore, prevStored]);
    ...
    }, [apiUrl, fade, fetchCallback, timeout]);

    useEffect(() => {
        if (nextInterval > 0) {
            timeout.current = window.setTimeout(fetchForumData, nextInterval * 1000);
        }
    }, [fetchForumData, nextInterval, posts]);

...
const GameListComponent: React.FC<GameListProps> = (props) => {
    ...
    const fetchGamesData = useCallback(() => {
        ...
        apiGet<IListData>(apiUrl)
            .then(function(data) {
                setLoaded(true);
                if (data.model) {
                    setGames(data.model.games);
                    setLoading(false);

                    if (data.model.nextInterval > 0) {
                        timeout.current = window.setTimeout(fetchGamesData, data.model.nextInterval * 1000);
                    }
                }
            })
